...
module Persistent = struct
  ...
  let create' ~server_name ?on_event ?retry_delay ?buf ?hb_ns streams =
    create ~server_name ?on_event ?retry_delay
      ~connect:(fun _ -> connect ?buf ?hb_ns streams)
end
...
let with_connection ?buf ?hb_ns streams ~f =
  Deferred.Or_error.bind (connect ?buf ?hb_ns streams) ~f:begin fun r ->
